...
// 读取JSON文件
$.getJSON("../data/g3_tingxie.json", "", function(data) {
    datahouse = data
    // 默认上册
    makeChapter(data)
})
...
function restartTest() {
    ...
    //layui.element.progress('progress', percent)
...
function readWord(word) {
  ...
  /*  // 检查缓存是否存在
    ...
    var cachefilename = 'cache' + md5 + '.mp3'

    var cachefilepath = '../data/cache/' + cachefilename
    var exists = fileexists(cachefilepath)
    if(exists){
        // 播放文件
        playVoice(cachefilepath)
    }
    else{
        datatemplate.text = word
        var data = querystring.stringify(datatemplate)
        option.path = '/text2audio?' + data
        ...
        var req = http.request(option,function(res){
            ...
            res.on('end',function(){
                ...
                var filename = './app/data/cache/' + cachefilename
